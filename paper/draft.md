...
We present the first SNN evaluation on the In-the-Wild benchmark, using an exploratory design crossing frontend (mel spectrogram vs. wavelet scattering), backend (ResNet-18 vs. SNN), and augmentation condition to test this prediction and explore what drives cross-domain generalization. Our contributions are:
